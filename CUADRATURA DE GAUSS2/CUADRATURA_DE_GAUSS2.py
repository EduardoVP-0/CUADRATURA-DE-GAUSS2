...
mtp.plot(x_v1, y_v1,label='Funcion')
mtp.plot(x_v, y_v,label='Cuadratura')
mtp.scatter([a],funcion.subs(x,a),color="red")
mtp.scatter([b],funcion.subs(x,b),color="red")
mtp.fill_between(x_v, 0, y_v, where=(x_v >= a) & (x_v <= b), alpha=0.5, color='gray', label='Area bajo la curva')
mtp.title("Grafica")
mtp.xlabel("x")
mtp.ylabel("f(x)")
mtp.legend()
